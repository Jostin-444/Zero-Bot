...
const groq = new Groq({ apiKey: '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' });
    ...
    if (!ya) throw `\`\`\`[ 🍄 ] Por favor ingresa un texto. Ejemplo: ${usedPrefix + command} Hola\`\`\``
      try {
        ...
        conn.sylph[m.sender] = [{
          role: 'system',
          content: `Eres Zero, una inteligencia artificial creada por Zero Two, responde de manera clara y concisa para que los usuarios entiendan mejor tus respuestas. El nombre del usuario será: ${conn.getName(m.sender)}`,
        }];
